...
const getDocByID = async(doc_num) => {
    try{
        sql.close();
        let pool = await sql.connect(config.sql);
        const sqlQueries = await utils.loadSqlQueries('Computer');
        const event = await pool.request()
                            .input('doc_num', sql.NVarChar(50), doc_num)
                            .query(sqlQueries.getdoc_num);
        return event.recordset;
    }catch (error) {
        return error.message;
    }
...
const insertRepairLog = async(data) => {
    try {
        ...
        const insert = await pool.request()
                            ...
                            .input('doc_num', sql.NVarChar(50), data.doc_num)
...
const updatecombroken = async(case_service, data) => {
    try{
        sql.close();
        let pool = await sql.connect(config.sql);
        const sqlQueries = await utils.loadSqlQueries('Computer');
        const update = await pool.request()
                            .input('case_service', sql.NVarChar(50), case_service)
                            .input('update_by', sql.NVarChar(50), data.update_by)
                            .query(sqlQueries.updateComBroken);
        return update.recordset;
    }catch (error) {
        return error.message
    }
...
const getComBroken = async () => {
    try{
        ...
        let pool = await sql.connect(config.sql);
        const sqlQueries = await utils.loadSqlQueries('Computer');
        const getList = await pool.request().query(sqlQueries.getComBroken);
        return getList.recordset;
    }catch (error) {
        return error.message;
    }
