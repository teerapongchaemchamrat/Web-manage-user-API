...
const computerData = require('../data/CYF/Computer');
...
const GetDocById = async ( req, res, next) => {
    try{
        ...
        const event = await computerData.getDocByID(doc_num);
        res.send(event);
    }catch (error) {
        res.status(400).send(error.message);
    }
...
const updateComBroken = async (req, res, next) => {
    try {
        const case_service =  req.params.case_service;
        const data = req.body;
        const updated = await computerData.updatecombroken(case_service, data);
        res.send(updated);
    } catch (error) {
        res.status(400).send(error.message);
    }
...
const GetComBroken = async (req, res, next) => {
    try{
        const getall = await computerData.getComBroken();
        res.send(getall);
    }catch (error) {
        res.status(400).send(error.message);
    }
